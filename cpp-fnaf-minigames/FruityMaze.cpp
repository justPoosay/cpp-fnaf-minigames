...
static const char* shaderPath = "FruityMaze.fs";
...
static void AddScore(int points, const FruityMazeGameResources& resources) {
    ...
    if (newBonuses > oldBonuses) {
        ...
        // **NEW: Start the time extended animation**
...
static void UpdateGameTimer(float deltaTime, const FruityMazeGameResources& resources) {
    ...
    if (gameState.gameTimer <= 10 && gameState.gameTimer > 0) {
        ...
        if (currentSecond != gameState.lastTickSecond && currentSecond <= 10) {
            ...
            PlaySound(resources.timerPingSound);
        }
    }
    ...
    if (gameState.gameTimer <= 0) {
        ...
        textGif.timeIsUpTimer = 1;

        // **NEW: Start the time is up animation**
...
static void UpdateGifText(float deltaTime) {
    if (textGif.showTimeExtended) {
        ...
        if (textGif.timeExtendedTimer <= 0) {
            textGif.showTimeExtended = false;
        }
    }

    if (textGif.showAmazing) {
        textGif.amazingTimer -= deltaTime;
        if (textGif.amazingTimer <= 0) {
            textGif.showAmazing = false;
        }
    }

    if (textGif.showTimeIsUp) {
        textGif.timeIsUpTimer -= deltaTime;
        if (textGif.timeIsUpTimer <= 0) {
            textGif.showTimeIsUp = false;
        }
    }
}

// **NEW: Animation update function**
    ...
    for (auto& prop : gameProps) {
        if (!prop.collected) {
            ...
            if (CheckCollisionRecs(playerHitbox, propHitbox)) {
                ...
                if (prop.type == PROP_LIGHTNING ||
                    prop.type == PROP_GUMMYBEAR ||
                    prop.type == PROP_MAGNET)
                    ...
                    if (AreAllFruitsCollected() && !gameState.gameOver) {
                        ...
                        // **NEW: Start the amazing animation**
...
static void DrawProps(const FruityMazeGameResources& resources, float time, const Camera& camera, bool debugMode = false) {
    ...
    for (const auto& prop : gameProps) {
        if (!prop.collected) {
            ...
            if (modelAvailable && modelToDraw) DrawModelEx(*modelToDraw, drawPosition, { 0, 1, 0 }, prop.rotationY, { propScale, propScale, propScale }, WHITE);
            else {
...
// **MODIFIED: New sprite sheet animation version of DrawTextGif**
static void DrawTextGif(const FruityMazeGameResources& resources) {
    ...
    // **NEW: Draw TIME EXTENDED animation**
    if (textGif.showTimeExtended && resources.timeExtendedAnim.spriteSheet.id > 0) {
        ...
        float texWidth = sourceRect.width * scale;
        float texHeight = sourceRect.height * scale;
    ...
    // **UNCHANGED: Static out of bounds texture**
    if (textGif.showOutOfBounds && resources.outOfBounds.id > 0) {
        float texWidth = resources.outOfBounds.width * scale;
        float texHeight = resources.outOfBounds.height * scale;
...
int runFruityMaze(GraphicsQuality quality, Shader postProcessingShader, bool applyShader) {
    TraceLog(LOG_DEBUG, TextFormat("Starting Fruity 3D Maze | Quality: %d | Shader: %d", quality, applyShader));
    HideCursor();
    ...
    while (!WindowShouldClose()) {
        ...
        if (startFound && resources.mapPixels && !gameState.gameOver) {
            ...
            if (Vector3LengthSqr(moveDirectionInput) > 0) {
                ...
                if (!powerUps.gummybearActive && !gameState.temporaryNoclip)
                    canMove = !IsColliding(tempPos, resources.mapModelPosition, resources.mapWidth, resources.mapHeight, resources.mapPixels, playerRotationAngle);
                ...
                for (int x = -1; x <= 1; x++) {
                    for (int z = -1; z <= 1; z++) {
                        ...
                        if (wallCheckMapX >= 0 && wallCheckMapX < resources.mapWidth &&
                            ...
                            if (x == 0 && z == 0) DrawCubeWires({ resources.mapModelPosition.x + wallCheckMapX, -0.4, resources.mapModelPosition.z + wallCheckMapZ }, 2, 0.2, 2, BLUE);
                            else DrawCubeWires({ resources.mapModelPosition.x + wallCheckMapX, -0.45, resources.mapModelPosition.z + wallCheckMapZ }, 2, 0.2, 2, RED);
                        }
                    }
                }
        ...
        else posMinimap = { resources.mapModelPosition.x + resources.mapWidth / 2, 0, resources.mapModelPosition.z + resources.mapHeight / 2 };
        ...
        if (startFound) {
            ...
            DrawCircle((minimapVisualTopLeft.x + playerRelXOnMinimap), (minimapVisualTopLeft.y + playerRelZOnMinimap), collisionRadius * minimapScale, playerDotColor);
        ...
        if (startFound) {
            ...
            if (powerUps.lightningActive) {
                ...
                DrawText(lightningText, virtualScreenWidth - lightningWidth - textPadding, powerUpYOffset, UI_LARGE_FONT_SIZE, lightningColor);
            ...
            if (powerUps.gummybearActive) {
                ...
                DrawText(gummybearText, virtualScreenWidth - gummybearWidth - textPadding, powerUpYOffset, UI_LARGE_FONT_SIZE, gummybearColor);
                powerUpYOffset += UI_LARGE_FONT_SIZE + 10;
            }
            ...
            if (powerUps.magnetActive) {
                ...
                DrawText(magnetText, virtualScreenWidth - magnetWidth - textPadding, powerUpYOffset, UI_LARGE_FONT_SIZE, magnetColor);
                powerUpYOffset += UI_LARGE_FONT_SIZE + 10;
            }
            ...
            // Simplified game over display - visual feedback handles main messages
            if (gameState.gameOver) {
            ...
            if (debug) controlsText = "ESC: Return to Menu | T: Switch Camera | F3: Debug | F5: Reload Shader | L: Speed | G: Noclip | M: Magnet";
            else controlsText = "ESC: Return to Menu | T: Switch Camera | F3: Debug | F5: Reload Shader";
